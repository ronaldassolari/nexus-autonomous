#!/bin/bash
# Script de contenção para fileproviderd (FileProvider Daemon)
# Nexus Orchestrator - Monitoramento Intensivo
# Criado em resposta à crise de 2026-03-30 01:03 AM

LOG_FILE="fileproviderd_monitor.log"
MAX_CPU=50  # Limite máximo de CPU permitido para fileproviderd
CHECK_INTERVAL=15  # Verificar a cada 15 segundos
MAX_KILL_ATTEMPTS=2  # Máximo de tentativas de kill por ciclo

echo "=== Iniciando Monitoramento FileProviderd ===" > "$LOG_FILE"
echo "Data/Hora: $(date)" >> "$LOG_FILE"
echo "Limite CPU: ${MAX_CPU}%" >> "$LOG_FILE"
echo "Intervalo: ${CHECK_INTERVAL}s" >> "$LOG_FILE"
echo "Motivo: Crise detectada em 2026-03-30 01:03 AM (81.7% CPU)" >> "$LOG_FILE"
echo "" >> "$LOG_FILE"

while true; do
    TIMESTAMP=$(date '+%Y-%m-%d %H:%M:%S')
    
    # Encontrar processos fileproviderd
    PROCESSES=$(ps aux | grep fileproviderd | grep -v grep | grep -v "$0")
    
    if [ -n "$PROCESSES" ]; then
        echo "[$TIMESTAMP] Processos fileproviderd encontrados:" >> "$LOG_FILE"
                ...
                if [ "$KILL_COUNT" -lt "$MAX_KILL_ATTEMPTS" ] && [ "$USER" != "root" ]; then
                    ...
                    if ps -p "$PID" > /dev/null 2>&1; then
                        ...
                        if ! ps -p "$PID" > /dev/null 2>&1; then
                            ...
                            CRISIS_LOG="crises_fileproviderd.log"
                            echo "[$TIMESTAMP] CRISE: fileproviderd eliminado - CPU: ${CPU}%, PID: $PID" >> "$CRISIS_LOG"
                            
                            # Registrar também no log geral de crises
                            echo "[$TIMESTAMP] CRISE FILEPROVIDERD: CPU ${CPU}%, PID $PID" >> "crises_geral.log"
                        else
                        ...
                        # Registrar no log de crises
                        CRISIS_LOG="crises_fileproviderd.log"
                        echo "[$TIMESTAMP] CRISE: fileproviderd terminado - CPU: ${CPU}%, PID: $PID" >> "$CRISIS_LOG"
        ...
        echo "[$TIMESTAMP] Nenhum processo fileproviderd ativo" >> "$LOG_FILE"
    fi
    ...
    # Verificar se há múltiplos processos fileproviderd (possível vazamento)
    FILEPROVIDERD_COUNT=$(ps aux | grep fileproviderd | grep -v grep | grep -v "$0" | wc -l)
    if [ "$FILEPROVIDERD_COUNT" -gt 2 ]; then
        echo "[$TIMESTAMP] ALERTA: $FILEPROVIDERD_COUNT processos fileproviderd detectados (possível vazamento)" >> "$LOG_FILE"
    fi
